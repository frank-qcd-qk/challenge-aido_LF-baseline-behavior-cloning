...
class cbcNet:
    ...
    def build_cbc_net(rgb_image):
        ...
        def anomaly_controller(x):
            x = Dense(500, kernel_initializer='normal', activation='relu', name='ANB_FC1')(x)
        ...
        def lane_following_controller(y):
            y = Dense(500, kernel_initializer='normal', activation='relu', name='BCB_FC1')(y)
        ...
        prediction = Switch(Less_equal(anomaly, 0.5), anomaly_controller(prediction),
                            lane_following_controller(prediction), name="prediction")
